...
def savefunction(filename,image_info,data):
    ...
    cur.execute(''' INSERT INTO image_image(filename,image_info,data,project_id)
              VALUES(?,?,?,?) ''', (filename, json.dumps(image_info), json.dumps(data),45,))
    conn.commit()
    print('commit')
    #print(filename, image_info, data)

bbox_json = open("/media/hkuit164/Backup/pose_thermal/result.json", "r")
bbox_data = json.load(bbox_json)

keypoint_json = open("/media/hkuit164/Backup/pose_thermal/result1.json", "r")
...
for b_data, k_data in zip(bbox_data, keypoint_datas[0]["preds"]):
    ...
    image_name = b_data['image_path'].replace('/media/hkuit164/Backup/label_studio_consise/label_studio_consise/media/project_45/','')
    if tem_name != b_data['image_path']:
        ...
        filename = 'project_45/' + image_name
